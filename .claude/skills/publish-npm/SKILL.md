---
name: publish-npm
description: Publica o pacote specd no registry npm usando o token do .env, com gate verde, dry-run revisado, confirmação explícita do autor e releitura do registry como prova. Use quando o usuário pedir para publicar no npm, subir versão, lançar release, reservar o nome no registry, rodar npm publish, ou perguntar como fazer `npx specd` funcionar de verdade — e também quando pedir só o dry-run ou verificar o que iria no tarball.
---
...
já pagou esse preço: `@jnerytech/specd@0.0.0` está publicado desde
2026-07-28, e o `package.json` em `HEAD` diz `"name": "specd"` — o escopo
entrou em `5a21f2a` e foi desfeito em `37a35ff`. Publicar daqui hoje não
atualiza o pacote que existe; reserva um segundo nome, e quem instalou o
primeiro continua com o primeiro.

Se os dois nomes divergirem, pare e traga a escolha ao autor: atualizar o
escopado exige `"name": "@jnerytech/specd"` no `package.json`, e publicar o
não escopado é decisão nova sobre qual nome o projeto passa a ter.
...
- Primeira publicação de reserva de nome: `0.0.0` serve e é o que o README já
  descreve.
...
- `README.md` e `CLAUDE.md` afirmam que o pacote **não está publicado** e que
  `npx specd` responde 404. Metade disso já é mentira: `@jnerytech/specd` está
  publicado desde 2026-07-28 — quebrado, sem `dist/`, mas ocupando o nome. O
  que continua verdade é o 404 do `specd` sem escopo. Ofereça a correção nos
  dois arquivos, e ela depende de qual nome o projeto assume.
- Se houve `npm version`, a tag ainda é local: `git push --follow-tags`.
- Há teste amarrando o caminho citado no README ao `bin` do `package.json` —
  se o README mudar, rode `npm test` antes de commitar.
