...
- Nenhuma asserção nova sobre estado do registry, em nenhuma redação. É a regra do requisito aplicada à task que a implementa: `verify` é offline por gate-no-network, então essa pergunta não é do gate
...
- Não reabrir a decisão de escopo. Renomear pacote publicado tem custo próprio e não é desta change; o README pode registrar a opção sem tomá-la
