...
# published-package-documentation — a publicação que aconteceu e a documentação que não soube
...
por gate-no-network e o registry é rede. Então o que parecia rede era fecho: manteve a frase
no lugar exatamente enquanto ela deixava de ser verdade.

**Este é absence-is-not-compliance numa forma que ainda não estava na lista.** As quatro instâncias
...
Hoje as duas são 0.0.2 e a divergência não aparece. Quando a change `help-and-version-surface` entrar,
`node_modules/.bin/specd verify --help` sai 2 e `dist/cli.js verify --help` sai
0 — mesmo nome de comando, respostas diferentes.

O modo de falha é absence-is-not-compliance: um gate que resolvesse pelo PATH do npm rodaria a versão
...
absence-is-not-compliance na direção de escrita — resposta de sucesso não é prova, quem confirma é a
...
migrar depois, é operação de registry: fica fora do gate por gate-no-network e fora desta
change.

**Não acrescenta teste sobre o registry.** É a regra desta change aplicada a ela
mesma. Um teste que consultasse `npm view` poria rede no gate e violaria gate-no-network; um
que afirmasse a publicação em prosa recriaria a linha 52 com outro texto.

## Fora de escopo

`propose`, `apply` e memória. A change `help-and-version-surface`, que está aberta e não arquivada — as
