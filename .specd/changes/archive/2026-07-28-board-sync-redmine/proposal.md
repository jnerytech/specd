...
# board-sync-redmine — sync: o adaptador de board

## Por quê

Cinco changes produziram um gate que lê a spec e o código. O board — onde o
...
lê. absence-is-not-compliance se aplica inteiro: não conseguir ler a definição não é o mesmo que o
...
decide —, então não é no-guessing-on-conflict, e resolver é correto. Mas é escrita destrutiva, então
sai no relatório com nome próprio em vez de virar um "atualizado" indistinguível
de qualquer outro.

Conflito sai **2**, não 1. single-gate: só `verify` reprova. `sync` não conseguir
...
`describeFields` é impedimento do absence-is-not-compliance. Quarta vez que aquele documento erra, e a
primeira em que ele erra no desenho central de uma change.

A falta é maior e não dá para contornar: **quatro escritas não bastam.** O
merge de três vias precisa ler o estado do board (`read`), e o absence-is-not-compliance precisa ler a
