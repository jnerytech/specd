...
# provenance-and-mcp-transport — provenance, transporte MCP e delta ilegível

## Por quê

A change `archive-cycle-and-effective-specs` fechou o ciclo e deixou três buracos, todos conhecidos e nenhum
acidental.
...
implementação da change `verify-gate-and-anchor-ladder` lê o modo de resposta JSON e marca a fonte como falha
em SSE — comportamento correto, sem requisito que o sustente.

E o achado da própria change `archive-cycle-and-effective-specs`: **delta ilegível passa por delta vazio.**
Arquivar a change `verify-gate-and-anchor-ladder` saiu com código 0 tendo escrito zero arquivos, porque o
...
perigoso que a change anterior expôs.
...
Primeira do produto sem rastreio paralelo em OpenSpec. O ciclo que a change `archive-cycle-and-effective-specs`
...
**Resolvido na change `archive-cycle-and-effective-specs` — `anchor fix`.** Entrou: `src/anchors/fix.ts` existe,
`REQ-ANC-008` está realizado com exit 2, e há teste. Não era ponta solta.

## Critério de sucesso

`specd verify` sai 0 sem nenhum warning, com a change `provenance-and-mcp-transport` arquivada e as seis
