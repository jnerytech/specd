...
# archive-cycle-and-effective-specs — fechar o ciclo change → verify → archive

## Por quê

A change `verify-gate-and-anchor-ladder` entregou o gate e não entregou como fechar uma change. O resultado é
...
Esta change fecha o ciclo. O critério de sucesso é auto-aplicação: `specd archive`
arquiva a change `verify-gate-and-anchor-ladder`, depois arquiva a change `archive-cycle-and-effective-specs`, e `specd verify` fica verde sem
warning.
...
`explore/manifest.json`. A change `archive-cycle-and-effective-specs` não é dirigida por card e não tem bundle:
ligar `provenance` aqui reprovaria esta própria change. O requisito espera a
change `provenance-and-mcp-transport`, junto com o transporte MCP, e mora no delta de lá.

## Auto-aplicação

O alvo fácil vem antes do difícil e prova a mesma coisa:

1. `specd archive 2026-07-28-verify-gate-and-anchor-ladder` sai 0. Os requisitos afetados da change `verify-gate-and-anchor-ladder` têm
   âncora resolvendo.
2. change `archive-cycle-and-effective-specs` implementada. Gate verde com os warnings desta change.
3. `specd archive 2026-07-28-archive-cycle-and-effective-specs` sai 0.
4. `specd verify` verde, sem warning, com a change `provenance-and-mcp-transport` ainda aberta.
