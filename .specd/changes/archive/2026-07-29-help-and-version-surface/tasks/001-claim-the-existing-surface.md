...
- Vem primeiro porque 002 e 003 mexem em `USAGE` e no dispatch, e refatorar sob rede é a diferença entre notar uma regressão e descobri-la em outra change
- Rodar num workspace sem `.specd/` é o ponto: um diretório sem spec faz `verify` sair 2 por absence-is-not-compliance instância 1, então se algum caminho de help lesse spec o teste falharia por isso
