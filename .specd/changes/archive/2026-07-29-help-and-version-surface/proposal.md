...
# help-and-version-surface — a superfície de primeiro contato, que ninguém reivindicou
...
uma terceira cópia — a doença que esta change trata.

`specd help` sem argumento continua existindo, porque já existe e REQ-CLI-008 o
declara.

## single-gate, absence-is-not-compliance e costly-ops-are-not-silent

**single-gate.** Toda essa superfície sai 0 ou 2, nunca 1. Está escrito como critério e
...
**absence-is-not-compliance é o motivo do buraco 1 estar nesta change e não numa lista de melhorias.**
...
**costly-ops-are-not-silent não morde.** Help não escreve, não toca rede, não custa. É a operação mais
...
pipe — config-only-on-divergence corta os dois, sem dois clientes divergindo. `--cwd` global: o README
resolve com `cd`, e mexer na origem do `cwd` é vizinhança de absence-is-not-compliance instância 1
(rodar do diretório errado virava aprovação), que não se toca de passagem.
"Did you mean" em comando desconhecido: não fere no-guessing-on-conflict — sugerir não é
auto-resolver, e `anchor suggest` é precedente — mas é conveniência, não buraco.

`propose`, `apply` e memória, como sempre.

## Achado lateral, não desta change
