...
# project-root-and-file-visibility — fazer o specd funcionar fora deste repositório

## Por quê

As três primeiras changes foram validadas contra um único repositório: este. Ele
...
Isso virou absence-is-not-compliance: **ausência de dado não é conformidade.** A terceira instância do
mesmo modo de falha em três changes.
...
requirement is in the active change delta"* — que REQ-ANC-006 aposentou na change
`archive-cycle-and-effective-specs`. Todo projeto criado desde então aprendeu a regra errada.

O texto está corrigido nesta change. O que não está resolvido é a classe: um
template é prosa dentro de código, e nenhum requisito pode afirmar que ele
descreve o comportamento vigente sem um checador semântico, que no-llm-in-decision-path mantém fora
