...
# Fatia 5 — hooks: o que torna o gate obrigatório

## Por quê

Quatro fatias produziram um gate que funciona e que ninguém é obrigado a rodar.
`specd verify` só reprova quem o chama. Enquanto chamá-lo for opcional, a
detecção de drift é uma sugestão, e a fatia anterior mostrou o que acontece com
...
preferência, então P5 passa. Mas a escolha fica legível no arquivo que o host lê,
...
que ele existe em vez de o `settings.json` chamar `verify` direto. P2 fica
...
é P8 na forma mais pura.

E P8 de novo no outro sentido: não conseguir rodar o gate (não é projeto specd,
...
**Os dois estão superados, por dois achados independentes desta fatia.** O `Stop`
...
o desenho previa antes do contato com o campo, e a Fatia 5 é a primeira change que
...
nome de símbolo continua proibido por P4.

E P8 vale aqui também: extensão sem padrão de declaração conhecido reporta isso.
...
Fica registrado como run de sandbox: `sandbox/runs/003-fatia-5.md`, com a versão do
