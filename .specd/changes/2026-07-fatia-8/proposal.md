...
# Fatia 8 — a órfã que a Fatia 7 isentou, e o intervalo que ninguém representa

## Por quê

Dois achados da exploração dos dois momentos de virar real. São pequenos, são
da mesma família, e um deles é a Fatia 7 corrigindo o sintoma e o sintoma
...
006, no caminho que a Fatia 7 isentou por construção.

Pela letra de P9 o autor declarou uma morte e recebeu uma morte. Pelo teste de
P9 — o custo é visível no momento em que é pago — escrever `- REQ-002` numa
...
da correção da Fatia 7. Ler o sinal destrava o `sync` sem decidir a pergunta dos
...
destrutiva numa direção só, e é P4 literal: dois estados possíveis, indício
...
Silenciar seria P8 pela definição: verificou, encontrou algo, e não contou. O
...
## P9 e P1, já aplicados

P1 ganhou a segunda justificativa em AGENTS.md e CLAUDE.md, escrita antes desta
fatia porque governa como ela é construída: determinismo protege o exit code,
...
**Primeira razão, da Fatia 7.** Renomear volta a ser grátis de verdade, sem
...
de transição no delta, `RENAMED` e `SPLIT` — é a que P1 estendido rejeita. Se a
...
P1 na letra, porque exit code nenhum depende disso; é P1 na razão, e com força
