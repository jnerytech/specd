...
Pointer spec. The authoritative behavior contract lives in `.specd/specs/` — this file maps change `verify-gate-and-anchor-ladder` implementation work to those REQ-IDs by reference and MUST NOT restate or redefine them. Acceptance criteria of each referenced requirement are the test specification.

## ADDED Requirements

### Requirement: Implementation conforms to referenced .specd requirements

The implementation SHALL satisfy every requirement referenced by the nine tasks of `.specd/changes/2026-07-28-verify-gate-and-anchor-ladder/tasks/`, as written in `.specd/specs/`.
