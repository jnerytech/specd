# Design — verify-gate-implementation
...
- no-llm-in-decision-path/gate-no-network: architecture tests walk the import graph from `src/verify/index.ts` and fail on LLM or network modules (task 007).
- single-gate/exit codes: only `verify` exits 1 for quality; operational failures exit 2.
- no-guessing-on-conflict: ambiguity → error with diagnostic, never auto-resolution.
