...
**It cannot recur.** Once change `archive-cycle-and-effective-specs` ships, a requirement enters `specs/` only through `archive`, and `archive` refuses to run while any affected anchor dangles (REQ-ANC-007). The illegal state stops being reachable. **No fourth delta section will ever be needed** — not for deferral, which `delta-format-and-package-identity` already removed, and not for migration, which this change performs once and closes.
...
- Eight move to `.specd/changes/2026-07-28-archive-cycle-and-effective-specs/delta.md` as ADDED: REQ-ANC-007, REQ-ANC-008, REQ-VER-004, REQ-VER-005, REQ-FMT-004, REQ-FMT-005, REQ-FMT-006, REQ-FMT-007. All are implemented by change `archive-cycle-and-effective-specs`.
- REQ-VER-003 moves to `.specd/changes/2026-07-28-provenance-and-mcp-transport/delta.md`. change `archive-cycle-and-effective-specs` cannot hold it: as written it rejects any change lacking `explore/manifest.json`, which would reject change `archive-cycle-and-effective-specs` itself. change `provenance-and-mcp-transport` exists so the requirement has a home; see its `proposal.md`.
- REQ-EXP-007 does **not** move. It is realized — vacuously, since nothing validates `draft.md` — and its dangling anchor was an anchor error, not missing behaviour. **It loses its anchor entirely.** A negative requirement has nothing to point at, and an anchor aimed at the verifier would resolve while proving nothing. Decorative anchor is worse than absent anchor: it trades honest silence for a false signal of coverage. anchor-necessary-not-sufficient, and REQ-ANC-001 already makes anchors optional.
...
- REQ-ANC-008 changes exit code 1 to 2. "No suggestion to apply" is a refusal to act, not a verdict, and exit 1 belongs to `specd verify` alone (single-gate).
- REQ-VER-005 splits into three, one per outcome the maintainer decided: empty `evidence.commits` on a `done` task rejects (REQ-VER-005); a listed SHA unreachable in history warns (REQ-VER-010); an unavailable history exits 2 (REQ-VER-011). Anchors prove code now, evidence proves work then — different axes, anchor-necessary-not-sufficient. The rule survives squash, rebase and shallow clone while keeping the anti-fraud property that matters: a completion claim with no support at all still fails.

**MODIFIED in change `archive-cycle-and-effective-specs`'s delta (text stays in `specs/` until archive)**
...
- REQ-CLI-001 and REQ-CLI-004: **exit 1 is a verdict, exit 2 is a refusal to act.** Without this, `archive` refusing on a coverage failure and `anchor fix` refusing without a suggestion both contradict single-gate in silence.
...
- REQ-CFG-008 and REQ-CFG-009: `specd status` reports how long each open change has been open and how many requirements it holds in warning. change `provenance-and-mcp-transport` will hold an orphan for an indefinite period, and a change that holds orphans without closing downgrades everything it lists — the same pathology this change corrects. These make stalling visible without the tool passing judgement.

**Documentation**
- `AGENTS.md` and `CLAUDE.md` state Modelo B as a governing rule, so an agent implementing change `archive-cycle-and-effective-specs` does not write new requirements straight into `specs/`.

## Capabilities

### New Capabilities

- `modelo-b-migration`: pointer spec mapping this change's corrections to the REQ-IDs of `.specd/specs/` and of the change `archive-cycle-and-effective-specs` and change `provenance-and-mcp-transport` deltas, by reference. The authoritative contract is `.specd/`; this file restates nothing.

### Modified Capabilities

None. `openspec/specs/` stays empty by the decision taken when `verify-gate-implementation` was archived: the durable contract lives in `.specd/`, enforced by `specd verify`.
...
- `.specd/changes/2026-07-28-archive-cycle-and-effective-specs/` — new: `delta.md` (20 ADDED, 3 MODIFIED) and `proposal.md`.
- `.specd/changes/2026-07-28-provenance-and-mcp-transport/` — new: `delta.md` (1 ADDED) and `proposal.md`.
- `test/dogfood.test.ts` — rewritten. It asserted the gate was red; that was the change `verify-gate-and-anchor-ladder` criterion and is now false. It asserts the Modelo B invariant instead.
- `AGENTS.md`, `CLAUDE.md` — Modelo B and the change `archive-cycle-and-effective-specs` ordering.
...
The original product proposal — reported to hold 63 requirements under `REQ-SPEC-*`, `REQ-BOARD-*`, `REQ-MEM-*` and `REQ-SEC-*` prefixes — **is not in this repository.** `git ls-files` does not list it; `.reference/` holds only the openspec and compozy clones, which are gitignored. It has now produced four citations of requirements that do not exist here: REQ-SPEC-012 and a `synced_hash` mechanism during the change `verify-gate-and-anchor-ladder` review, REQ-SPEC-011 and REQ-MEM-005 during the Modelo B design.
