...
change `archive-cycle-and-effective-specs` SHALL deliver a `specd archive` that applies a change's delta to the capabilities and files the change away, verified by archiving the two changes that produced it.

#### Scenario: Archive applies a delta it read

- **GIVEN** REQ-ARC-003, REQ-ARC-004 and REQ-ARC-005 (`.specd/specs/archive.md`)
- **WHEN** `specd archive 2026-07-28-archive-cycle-and-effective-specs` runs
- **THEN** it exits 0, writes 23 ADDED sections and 3 MODIFIED replacements across five capabilities, creates `.specd/specs/archive.md`, and moves the change to `.specd/changes/archive/2026-07-28-archive-cycle-and-effective-specs`
...
- **THEN** it exits 0 with exactly one warning, for REQ-VER-003 held by the open change `provenance-and-mcp-transport`, and no error
...
change `archive-cycle-and-effective-specs` SHALL parse the delta and task artifacts so that every layer sees the same model.
...
- **GIVEN** REQ-CLI-003 (`.specd/specs/cli.md`) and no-guessing-on-conflict
...
change `archive-cycle-and-effective-specs` SHALL grade a dangling anchor by where its requirement is written rather than by which change names its identifier.
...
change `archive-cycle-and-effective-specs` SHALL check that declared requirements have owners and that completion claims have support.
...
change `archive-cycle-and-effective-specs` SHALL keep exit code 1 for the verdict of `specd verify` and use exit code 2 wherever another command cannot act.
...
change `archive-cycle-and-effective-specs` SHALL make every write of `specd archive` reviewable and recoverable.

#### Scenario: Nothing is staged or committed

- **GIVEN** REQ-ARC-007 (`.specd/specs/archive.md`) and no-guessing-on-conflict
...
- **GIVEN** REQ-ARC-008 (`.specd/specs/archive.md`) and memory-is-ephemeral
