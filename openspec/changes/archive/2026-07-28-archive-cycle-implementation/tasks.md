# Tasks — `archive-cycle-and-effective-specs`

Behaviour contract: the REQ-IDs of `.specd/specs/` and of
`.specd/changes/archive/2026-07-28-archive-cycle-and-effective-specs/delta.md`. Rationale: `design.md`.

The same work is tracked in `.specd/changes/archive/2026-07-28-archive-cycle-and-effective-specs/tasks/`, in
...
- [x] 5.4 Write ten tasks for change `archive-cycle-and-effective-specs` and one for change `provenance-and-mcp-transport`, so coverage has something to check
- [x] 5.5 `specd archive 2026-07-28-verify-gate-and-anchor-ladder` → exit 0
- [x] 5.6 `specd archive 2026-07-28-archive-cycle-and-effective-specs` → exit 0, six capability files written
- [x] 5.7 `specd verify` → exit 0, one warning, from the open change `provenance-and-mcp-transport`
...
- [x] 6.1 Record that step 5.5 passed vacuously — change `verify-gate-and-anchor-ladder`'s delta is a Modelo A manifest the new parser reads as empty
